module DiffusionTutorialUtilities

import PyPlot
using Compose
using Colors

export load_graph, load_graph_and_coords, load_graph_metadata

"""
This function takes the name of a dataset and reads the graph data associated with it.
"""
function load_graph(name::AbstractString)
    (rows,header) = readdlm(joinpath("data","$(name).smat");header=true)
    A = sparse(
...
"""
This function takes the name of a dataset and reads the graph data associated with it
as well as the layout information if you'd like that for visualization purposes. We
have precomputed layouts for all of the data.
"""
function load_graph_and_coords(name::AbstractString)
    A = load_graph(name)
    xy = readdlm(joinpath("data","$(name).xy"))
    return A, xy
end

function load_graph_metadata(name::AbstractString,metatype)
    return readdlm(joinpath("data","$(name).$(metatype)"))
end
...
function cgraphplot(A,xy,s,c,cmap)
    n = size(A,1)
    (ei,ej) = findnz(A)
    xmin,xmax = extrema(xy[:,1])
    ymin,ymax = extrema(xy[:,2])
    ...
    area = UnitBox(xmin,ymax,xmax-xmin,ymin-ymax,leftpad=1mm,rightpad=1mm,toppad=1mm,bottompad=1mm)
    graph = compose(context(), all_edges, linewidth(0.5pt), fill(nothing), stroke("black"), strokeopacity(0.5))
...
function cgraphplot(A,xy,set::Set{Int};opacity::Float64=0.5,lw=0.5pt,radius::Float64=0.75,filled=true,nodestroke=0.75pt)
    n = size(A,1)
    (ei,ej) = findnz(A)
    xmin,xmax = extrema(xy[:,1])
    ymin,ymax = extrema(xy[:,2])
    ...
    not_nodes = circle(xy[notverts,1],xy[notverts,2],[0.5*radius])
    
    plotnot = compose(context(), not_nodes, fill(colorant"white"), stroke(colorant"black"), linewidth(nodestroke))
    plotset = compose(context(), set_nodes, fill(colorant"red"), stroke(nothing))
    
    return compose(context(units=area), plotnot, plotset, graph)
end

function test_cgraphplot(A,xy,s,c,cmap)
    n = size(A,1)
    (ei,ej) = findnz(A)
    xmin,xmax = extrema(xy[:,1])
    ymin,ymax = extrema(xy[:,2])
    ...
    ccmap = [ RGB(cmap[i,1],cmap[i,2],cmap[i,3]) for i=1:size(cmap,1)]
    #plot = compose(context(), all_nodes, fill(vectorcmap(c,ccmap)), stroke(nothing))
    fillvec = [ccmap[end-rand(1:100)] for i=1:size(xy,1)]
    @show length(fillvec)
    @show size(A)
    @show size(xy)
    for i=1:length(fillvec)-55
        fillvec[i] = ccmap[end]
    end
    @show fillvec[1:5]
    @show typeof(fillvec)
    plot = compose(context(), all_nodes, fill(fillvec), stroke(nothing))
    return compose(context(units=area), plot, graph)
end
...
PyPlot.matplotlib[:pyplot][:register_cmap]("viridis",
    PyPlot.matplotlib[:colors][:ListedColormap](_viridis_data))
PyPlot.matplotlib[:pyplot][:register_cmap]("plasma",
    PyPlot.matplotlib[:colors][:ListedColormap](_plasma_data))
PyPlot.matplotlib[:pyplot][:register_cmap]("inferno",
    PyPlot.matplotlib[:colors][:ListedColormap](_inferno_data))
PyPlot.matplotlib[:pyplot][:register_cmap]("magma",
    PyPlot.matplotlib[:colors][:ListedColormap](_magma_data))
